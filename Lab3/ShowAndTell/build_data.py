...
tf.flags.DEFINE_string("image_dir", "data/flickr8k_images/",
                       "Image directory.")


tf.flags.DEFINE_string("captions_file", "data/flickr8k_captions/captions.json",
                       "Captions JSON file.")


tf.flags.DEFINE_string("output_dir", "data/tfdata", "Output data directory.")
...
tf.flags.DEFINE_string("word_counts_output_file", "data/tfdata/word_counts.txt",
...
def _bytes_feature(value):
  print(type(value))
  # print(value)
  #print(value.decode("utf-8-sig"))
  """Wrapper for inserting a bytes Feature into a SequenceExample proto."""

  if isinstance(value, str):
      value = bytes(value, "utf-8-sig")
  else:
      value = value
...
def _load_and_process_metadata(captions_file, image_dir):
  ...
  with tf.gfile.FastGFile(captions_file, "rb") as f:
    caption_data = json.load(reader(f))
  ...
  for image in caption_data["images"]:
      ...
      for sentence in image["sentences"]:
          caption=_process_caption(sentence["raw"])
          captions.append(caption)
      image_metadata.append(ImageMetadata(image_id, filename, captions))
      num_captions += len(captions)
      num_images +=1
      # print(ImageMetadata(image_id, filename, captions))
  print("Finished processing %d captions for %d images in %s" %
        (num_captions, num_images, captions_file))


  # # Extract the filenames. (id,file_name) pairs
  # id_to_filename = [(x["id"], x["file_name"]) for x in caption_data["images"]]
  #
  # # Extract the captions. Each image_id is associated with multiple captions. (image_id, [captions]) pairs
  # id_to_captions = {}
  ...
  #
  # # Process the captions and combine the data into a list of ImageMetadata. (image_id,filename,[captions])
...
def main(unused_argv):
  ...
  flickr_dataset = _load_and_process_metadata(FLAGS.captions_file,
                                                    FLAGS.image_dir)
  print("load and preprocess of metadata is done")

  # flickr_train_dataset = _load_and_process_metadata(FLAGS.train_captions_file,
  #                                                   FLAGS.train_image_dir)
  # flickr_val_dataset = _load_and_process_metadata(FLAGS.val_captions_file,
  #                                                 FLAGS.val_image_dir)
  ...
  train_cutoff = int(0.85 * len(flickr_dataset))
  val_cutoff = int(0.90 * len(flickr_dataset))

  train_dataset = flickr_dataset[0:train_cutoff]
  val_dataset = flickr_dataset[train_cutoff:val_cutoff]
  test_dataset = flickr_dataset[val_cutoff:]
